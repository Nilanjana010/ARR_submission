...
def attack(model_name, n):
        ...
        if model_name == "Llama-3.1-8B":
                ...
                pipeline = transformers.pipeline(
                    "text-generation", model=model_id, model_kwargs={"torch_dtype": torch.bfloat16}, device_map="auto", max_new_tokens= 8192
                )
                for i, instnce in enumerate(listD):
                    attack = instnce.strip()
                    response_llama31b = pipeline(attack)
                    list_queX.append("Q: " + attack)
                    str3 = response_llama31b[0].get('generated_text')
                    if str3.startswith(attack):  
                           str3 = str3[len(attack):].strip()
                    listX.append("A: " + str3)
                    print("response_llama3.1b = ", i, "  ", str3)
                ...
                for i, instnce in enumerate(listD):
                    ...
                    print("response_phi15 = ", i, "  ", response_phi15)
                ...
                for i, instnce in enumerate(listD):
                    ...
                    print("response_t5 = ", i, "  ", response_t5)
        elif model_name == "Mistral-7B-v0.1":
                nm = "mistralai/Mistral-7B-v0.1"
                model_nm = pb.LLMModel(model=nm, max_new_tokens=1000, temperature=0.0001)        
                ...
                for i, instnce in enumerate(listD):
                    ...
                    print("response_mistral = ", i, "  ", response_mistral)
